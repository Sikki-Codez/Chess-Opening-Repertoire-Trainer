# ♟️ Chess Opening Repertoire Trainer

An interactive, responsive React + Vite web application built to help chess players practice, train, and build custom opening repertoires. It combines real-time theory detection, interactive visual move trees, sound synthesis, and an integrated **Stockfish** engine to analyze deviations from standard opening theory.
...
*   **🌳 Interactive Visual Move Tree**: The app generates an interactive node tree (rendered via `react-d3-tree`) detailing the opening path. Click on any past move in the tree to jump the chessboard to that position.
